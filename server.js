...
app.get("/api/users", (req, res) => {
  ...
  client.query("SELECT * FROM users", (err, result) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Fel vid hämtning av användare: " + err });
    }
    // Kontrollera om några användare hittas och returnera alltid en array, även om den är tom
...
app.post("/api/users", (req, res) => {
  ...
  client.query(query, values, (err, result) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Fel vid skapande av användare: " + err });
    }
    return res
      .status(201)
      .json({ message: "User added", user: result.rows[0] });
  });
});
...
app.put("/api/users/:id", (req, res) => {
  ...
  client.query(query, values, (err, result) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Fel vid uppdatering av användare: " + err });
    }
    if (!result.rows || result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "Ingen användare hittad att uppdatera" });
    }
    return res
      .status(200)
      .json({ message: "User updated", user: result.rows[0] });
  });
});
  ...
  client.query(
    ...
    (err, result) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Fel vid borttagning av användare: " + err });
      }
      if (!result.rows || result.rows.length === 0) {
        return res
          .status(404)
          .json({ error: "Ingen användare hittad att ta bort" });
      }
      return res
        .status(200)
